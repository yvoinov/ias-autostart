...
# Oracle iAS Infrastructure autostart remove for Solaris 8,9,10,>10, Linux
# Yuri Voinov (C) 2006-2010
#
# ident "@(#)rm_infra.sh   2.5   10/11/01 YV"
#
...
SMF_XML_INFRA="$SVC_SHORT_NAME".xml
...
{
 # Check if uninstall config exists
 if [ ! -f "$RMV_CONFIG_FILE" ]; then
  $ECHO "Config file $RMV_CONFIG_FILE not found. Exiting..."
  exit 1
 else
  # Load uninstall config file into environment
  . $RMV_CONFIG_FILE
 fi
}
...
oratab_autostart_disable ()
{
 ...
 # Old code
 #$SED -e 's/.$//' -e 's/^[a-zA-Z0-9_-]*:.*:/&N/g' -e 's/^*:.*:/&N/g' $ORATAB>$TMP/oratmp && mv $TMP/oratmp $ORATAB
 # New code
 $SED -e 's/^[a-zA-Z0-9_+-]*:.*:/&N/' -e 's/^\*:.*:/&N/' -e 's/NY/N/' -e 's/NN/N/' $ORATAB>$TMP/oratmp && mv $TMP/oratmp $ORATAB
...
$ECHO "#####################################################"
$ECHO "#        iAS Infra autostart remove script          #"
$ECHO "#                                                   #"
...
if [ "$OS_FULL" = "SunOS 5.9" -o "$OS_FULL" = "SunOS 5.8" ]; then
 ...
 if [ "$OS_VER" -ge "10" ]; then
  ...
  if [ ! -f $SMF_DIR/$SMF_XML_MID ]; then
   ...
   $RM -f $SMF_DIR/$SMF_XML_INFRA>/dev/null 2>&1
  fi
  # Disabling autostart in oratab
  oratab_autostart_disable
  $ECHO "-------------------- Done. ------------------------"
