...
#
# ident "@(#)inst_midtier.sh    2.5   10/11/01 YV"
#
...
SMF_XML_LOC="$SVC_SHORT_NAME"_local.xml
BOOT_DIR="/etc/init.d"
SMF_DIR="/var/svc/manifest/application/oracle"
SVC_MTD="/lib/svc/method"

INST_CONFIG_FILE="inst_midtier.conf"
...
check_install_files ()
{
 ...
 if [ ! -f $SMF_XML -a ! -f $SMF_XML_LOC -a ! -f $SCRIPT_NAME -a ! -f $CONFIG_FILE_NAME -a ! -f $INST_CONFIG_FILE ]; then
...
make_smf ()
{
 ...
 if [ -f $SMF_DIR/$SMF_XML_INFRA ]; then
  ...
  $CP $SMF_XML_LOC $SMF_DIR/$SMF_XML
 fi 
...
$ECHO "#####################################################"
$ECHO "#     iAS Midtier autostart installation script     #"
$ECHO "#                                                   #"
