...
#
# ident "@(#)rm_midtier.sh    2.5   10/11/01 YV"
#
...
SVC2_SHORT_NAME="infra"

SCRIPT_NAME=init.ias_"$SVC_SHORT_NAME"
SMF_XML_INFRA="$SVC2_SHORT_NAME".xml
SMF_XML_MID="$SVC_SHORT_NAME".xml
BOOT_DIR="/etc/init.d"
...
$ECHO "#####################################################"
$ECHO "#       iAS Midtier autostart remove script         #"
$ECHO "#                                                   #"
...
if [ "$OS_FULL" = "SunOS 5.9" -o "$OS_FULL" = "SunOS 5.8" ]; then
 ...
 if [ "$OS_VER" -ge "10" ]; then
  ...
  $RM $CONFIG_FILE1>/dev/null 2>&1  # Remove config 1
  $RM $CONFIG_FILE2>/dev/null 2>&1  # Remove config 2
  # If infra is not installed on this host, remove directory,
  # otherwise remove XML file only
  if [ ! -f $SMF_DIR/$SMF_XML_INFRA ]; then
   $RM -R $SMF_DIR>/dev/null 2>&1
  else
   $RM -f $SMF_DIR/$SMF_XML_MID>/dev/null 2>&1
  fi
  $ECHO "-------------------- Done. ------------------------"
  $ECHO "Complete."
